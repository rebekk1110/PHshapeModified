...
def optim_bf_radius_gu(pers_1d: pd.DataFrame, bfr_0d: float, isDebug: bool = False) -> (float, pd.DataFrame):
    ...
    logging.info("Starting optim_bf_radius_gu")
    ...
    ##legger til debug
    #if isDebug:
    #    logging.debug(f"DBSCAN results: dbcls_num (pers) = {len(np.unique(cls_1d_perl.labels_))}, dbcls_num (death) = {len(np.unique(cls_1d_death.labels_))}")
    ...
    logging.info(f"Optimized buffer radius: {bfr_optm}")
...
def get_autooptim_bf_radius_GU(bldi_2d: np.ndarray, down_sample_num: float = 500,
    ...
    logging.info("Starting get_autooptim_bf_radius_GU")
    ...
    logging.info("Completed get_autooptim_bf_radius_GU")
