...
def simp_poly_Fd(poly:shapely.geometry,
    ...
    for pi in range(1 + len(poly_ints)):
        ...
        if not poly_simp_sele_list:
            if isDebug:
                print(f"[simp_basic_ol/simp_poly_Fd()] :: for poly_used[{pi}], "
                    f"no valid simplification found")
...
def simp_by_choosed_mtd(b_oli:shapely.geometry, method_name:str="haus", **kwargs):
    ...
    if method_name=="haus":
        try:
            ...
            raise e
        result = simp_poly_Fd(b_oli, thres_mode="haus", thres_haus=thres_haus, isDebug=isDebug)
        if result[0] is None:
            if isDebug:
                print("Warning: Simplification resulted in None")
            return None, None, None
        b_oli_simp_ext, b_oli_simp_ints, b_oli_simp = result
