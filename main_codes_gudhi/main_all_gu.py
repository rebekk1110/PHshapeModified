...
from utils.mdl_io import get_raster_path, get_output_folder, get_gt_shp_path, load_raster, save_buildings, load_buildings
from utils.mdl_visual import plot_buildings, plot_simplified_buildings, plot_gt_vs_simplified
from AlphaAlgorithm import process_tile_alpha
...
def main_all_gu(tile_name, force_rerun=False, algorithm='mdl1'):
    ...
    if algorithm == 'mdl1':
        ...
        # Save buildings as shapefile for evaluation
        gdf = gpd.GeoDataFrame(geometry=simplified_buildings)
        gdf.to_file(os.path.join(eval_folder, f"{tile_name}_buildings.shp"))
        res_type ='.shp'
...
if __name__ == "__main__":
    ...
    if results is not None and not results.empty:
        ...
        print("Processing completed with no valid results.")
